...
class LedgerInterface:
    ...
    def _parse_ledger_output(self, ledger_output: str) -> list[LedgerTransaction]:
        """Parse the output from 'ledger print' command."""
        transactions = []
        lines = ledger_output.split("\n")
        i = 0
        ...
        while i < len(lines):
            ...
            # Ledger CLI outputs dates in YYYY/MM/DD format
            if re.match(r"^\d{4}/\d{2}/\d{2}", line):
                transaction = self._parse_transaction(lines, i)
                if transaction:
                    transactions.append(transaction)
                    # Skip to after this transaction
                    i += 1 + len(transaction.postings)
                else:
                    i += 1
            else:
                i += 1
    ...
    def _parse_transaction(
        ...
        """Parse a single transaction starting at the given line index."""
        ...
        # Parse the transaction header: DATE [STATUS] DESCRIPTION
        # Status can be !, *, or empty
        # Ledger CLI outputs dates in YYYY/MM/DD format
        match = re.match(r"^(\d{4}/\d{2}/\d{2})\s*([!*]?)\s*(.*)$", first_line)
        if not match:
            return None
        ...
        while (
            i < len(lines)
            and lines[i].strip()
            and not re.match(r"^\d{4}/\d{2}/\d{2}", lines[i])
        ):
            posting = self._parse_posting(
                lines[i], i + 1
        ...
        return LedgerTransaction(
            ...
            description=description,
            status=status,
            line_number=start_idx + 1,  # 1-based line numbers
            postings=postings,
            original_line=first_line,
        )

    def _parse_posting(self, line: str, line_number: int) -> LedgerPosting | None:
        """Parse a single posting line."""
        if not line.strip():
            return None

        # Posting format: [SPACES][STATUS]ACCOUNT[SPACES][AMOUNT]
        # Status can be !, *, or empty and comes right after the initial spaces
        stripped = line.lstrip()
        if not stripped:
            return None

        # Check for status marker at the beginning
        status = ""
        ...
        # The account name comes first, amount (if present) comes at the end
        parts = stripped.split()
        if not parts:
            return None

        # Find where the account name ends and amount begins
        # Amount typically starts with $, -, or a digit and appears at the end
        # Look for the rightmost part that looks like an amount
        account_parts = parts
        amount_parts = []

        for i in range(len(parts) - 1, -1, -1):
            part = parts[i]
            if re.match(r"^[$-]?\d", part) or part.startswith("$"):
                # This looks like part of an amount, split here
                account_parts = parts[:i]
                amount_parts = parts[i:]
                break

        account = " ".join(account_parts)
        amount = " ".join(amount_parts) if amount_parts else ""
